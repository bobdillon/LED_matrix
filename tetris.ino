...
void blockchoice()
{
  ...
  int randomchoice= random(0,2);
   int randomstart=random(2,30);
   
  switch(randomchoice)
  {
    case 0://tower
    {
        newpiece.size[0][1]=myRED;
        newpiece.size[1][1]=myRED;
        newpiece.size[2][1]=myRED;
        newpiece.size[3][1]=myRED;
        newpiece.x =0;
        newpiece.y= randomstart;
...
void drawpixels(Piece newpiece)
{
   ...
   for(int i=0; i<4;i++)
    {
      for(int j=0;j<4;j++)
      {
        if(newpiece.size[i][j]==myRED)
        {
           display.drawPixel(  newpiece.x+i,newpiece.y+j,myRED );
        }
      }
    }
}
